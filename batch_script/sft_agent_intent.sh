#!/bin/bash
#SBATCH --job-name=fsdrive_sft_agent_intent
#SBATCH --partition=a100
#SBATCH --gres=gpu:4
#SBATCH --cpus-per-task=64
#SBATCH --time=1-00:00:00
#SBATCH --output=logs/sft_agent_intent_%j.out
#SBATCH --error=logs/sft_agent_intent_%j.err
...
TRAIN_DATA=/anvme/workspace/b305bb10-zyt/FSDrive/LlamaFactory/data/train_cot_motion_agent_intent.json
...
if [[ ! -f "$TRAIN_DATA" ]]; then
  echo "Missing agent-intent SFT data at $TRAIN_DATA" >&2
  exit 1
fi

cd /anvme/workspace/b305bb10-zyt/FSDrive/LlamaFactory

llamafactory-cli train ../configs/sft_agent_intent.yaml
