#!/bin/bash
#SBATCH --job-name=fsdrive_vllm_agent_intent_gt_prefix
#SBATCH --partition=a100
#SBATCH --gres=gpu:2
#SBATCH --cpus-per-task=32
#SBATCH --time=1-00:00:00
#SBATCH --output=logs/vllm_infer_agent_intent_gt_prefix_%j.out
#SBATCH --error=logs/vllm_infer_agent_intent_gt_prefix_%j.err
...
MODEL_PATH=saves/qwen2_vl-2b/sft_agent_intent_three_datasets
SAVE_NAME=eval_results/results_agent_intent_gt_prefix.jsonl

python ../tools/patch_vllm_rope_config.py "$MODEL_PATH"

python scripts/vllm_infer_gt_prefix.py \
  --model_name_or_path "$MODEL_PATH" \
  --dataset val_cot_motion_agent_intent \
