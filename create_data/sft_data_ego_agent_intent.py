...
system = "You're an autonomous vehicle's brain. Coordinates: X-axis is perpendicular, and Y-axis is parallel to the direction you're facing. You're at point (0,0). Units: meters. Based on the provided particulars, please output the latent driving intention, the latent surrounding vehicle intentions, the CAM_FRONT image at the 0.5 second in the future, and plan waypoints (0.5s intervals) for the next 3 seconds."
...
parser = argparse.ArgumentParser(description="Build SFT data with ego and surrounding vehicle intention supervision.")
parser.add_argument("--split", type=str, default="train", choices=["train", "val"])
parser.add_argument("--intent_labels", type=str, default="./create_data/intent_labels_k64.json")
parser.add_argument("--agent_intent_labels", type=str, default="./create_data/agent_intent_labels_k64_top4.json")
parser.add_argument("--output_suffix", type=str, default="ego_agent_intent")
...
for token in tokens:
    ...
    assistant_message = generate_assistant_message(data, token, traj_only=traj_only)
    ...
    human_value = (
        ...
        + user_message
        + "Based on the provided particulars, please output the latent driving intention, the latent surrounding vehicle intentions, the CAM_FRONT image at the 0.5 second in the future, and plan waypoints (0.5s intervals) for the next 3 seconds.\n"
    )
    assistant_value = (
        intent_token
        + " This is the latent driving intention. \n"
        + agent_intent_token
        + " These are the latent surrounding vehicle intentions. \n"
        + next_img_token
    ...
    num_language_tokens += len(encoding.encode(human_value, disallowed_special=()))
    num_user_tokens += len(encoding.encode(human_value, disallowed_special=()))
    num_language_tokens += len(encoding.encode(assistant_value, disallowed_special=()))
    num_assistant_tokens += len(encoding.encode(assistant_value, disallowed_special=()))
