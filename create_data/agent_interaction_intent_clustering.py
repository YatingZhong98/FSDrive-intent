...
def build_agent_future_positions(sample, agent_index, steps):
    boxes = np.asarray(sample["gt_boxes"], dtype=np.float32)
    feature = build_agent_feature(sample, agent_index, steps=steps, min_valid_steps=steps)
    offsets = feature.reshape(steps, 2)
    return boxes[agent_index, :2] + offsets


def build_ego_path(sample, steps):
    ego = np.asarray(sample["gt_ego_fut_trajs"], dtype=np.float32)
    mask = np.asarray(sample["gt_ego_fut_masks"], dtype=np.float32)

    if ego.shape[0] < steps + 1 or mask.shape[0] < steps:
        raise ValueError("ego trajectory is shorter than requested steps")
    if int(mask[:steps].sum()) < steps:
        raise ValueError("ego has too few valid future steps")
...
def interaction_metrics(ego_path, agent_future):
    aligned_distances = np.linalg.norm(agent_future - ego_path, axis=1)
    pairwise_distances = np.linalg.norm(agent_future[:, None, :] - ego_path[None, :, :], axis=2)
    aligned_index = int(aligned_distances.argmin())
    path_index = np.unravel_index(int(pairwise_distances.argmin()), pairwise_distances.shape)

    return {
        "aligned_distance": float(aligned_distances[aligned_index]),
        "aligned_step": aligned_index,
        "path_distance": float(pairwise_distances[path_index]),
        "agent_step": int(path_index[0]),
        "ego_step": int(path_index[1]),
    }
...
def select_agents(sample, args):
    ...
    try:
        ego_path = build_ego_path(sample, args.steps)
    except Exception:
        return selected
    ...
    for agent_index, name in enumerate(names):
        ...
        if not args.include_behind and y <= args.min_forward_y:
            continue
        ...
        try:
            ...
            agent_future = build_agent_future_positions(sample, agent_index, args.steps)
        except Exception:
            continue

        metrics = interaction_metrics(ego_path, agent_future)
        if metrics["path_distance"] > args.max_path_distance:
            continue
        if metrics["aligned_distance"] > args.max_aligned_distance:
            continue
    ...
    selected.sort(
        key=lambda item: (
            item["interaction"]["aligned_distance"],
            item["interaction"]["path_distance"],
            item["distance"],
        )
    )
...
def collect_features(data, tokens, args):
    ...
    if not features:
        raise ValueError("No valid interaction-agent trajectory features were collected.")
                ...
                entries.append(
                    {
                        ...
                        "path_distance": round(interaction["path_distance"], 4),
                        "aligned_distance": round(interaction["aligned_distance"], 4),
                        "agent_step": interaction["agent_step"],
                        "ego_step": interaction["ego_step"],
                        "aligned_step": interaction["aligned_step"],
...
def main():
    parser = argparse.ArgumentParser(
        description="Cluster ego-path-interacting vehicle trajectories into intent ids."
    )
    ...
    parser.add_argument("--max_distance", type=float, default=55.0)
    parser.add_argument("--max_lateral", type=float, default=20.0)
    parser.add_argument("--min_forward_y", type=float, default=-2.0)
    parser.add_argument("--max_path_distance", type=float, default=4.0)
    parser.add_argument("--max_aligned_distance", type=float, default=8.0)
    ...
    parser.add_argument(
        "--output_centers",
        default="./create_data/agent_interaction_intent_centers_k64.npy",
    )
    ...
    label_payload = {
        "metadata": {
            "selection": "ego_path_interaction",
            "num_clusters": args.num_clusters,
            "top_k": args.top_k,
            "steps": args.steps,
            "min_valid_steps": args.min_valid_steps,
            ...
            "max_aligned_distance": args.max_aligned_distance,
    ...
    train_with_interaction = len(set(train_sample_tokens))
    ...
    print(f"Train samples with interaction agents: {train_with_interaction}/{len(split['train'])}")
