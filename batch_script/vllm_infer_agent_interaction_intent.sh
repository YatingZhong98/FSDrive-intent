#!/bin/bash
#SBATCH --job-name=fsdrive_vllm_infer_agent_interaction_intent
#SBATCH --partition=a100
#SBATCH --gres=gpu:2
#SBATCH --cpus-per-task=32
#SBATCH --time=1-00:00:00
#SBATCH --output=logs/vllm_infer_agent_interaction_intent_%j.out
#SBATCH --error=logs/vllm_infer_agent_interaction_intent_%j.err
...
MODEL_PATH=saves/qwen2_vl-2b/sft_agent_interaction_intent
SAVE_NAME=eval_results/results_agent_interaction_intent.jsonl

python ../tools/patch_vllm_rope_config.py "$MODEL_PATH"

python scripts/vllm_infer.py \
  --model_name_or_path "$MODEL_PATH" \
  --dataset val_cot_motion_agent_interaction_intent \
