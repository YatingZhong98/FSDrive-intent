#!/bin/bash
#SBATCH --job-name=fsdrive_eval_ego_agent_intent
#SBATCH --partition=a100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --time=1-00:00:00
#SBATCH --output=logs/eval_ego_agent_intent_%j.out
#SBATCH --error=logs/eval_ego_agent_intent_%j.err
...
PRED_PATH=LlamaFactory/eval_results/results_sft_ego_agent_intent.jsonl
TOKEN_PATH=LlamaFactory/data/val_cot_motion_ego_agent_intent.json
EVAL_TRAJ_PATH=LlamaFactory/eval_results/eval_traj_sft_ego_agent_intent_val_cot_motion_ego_agent_intent.json

python tools/match.py \
  --pred_trajs_path "$PRED_PATH" \
  --token_traj_path "$TOKEN_PATH" \
...
python tools/evaluation/evaluation.py \
  --method FSDrive-ego-agent-intent \
  --metric uniad \
