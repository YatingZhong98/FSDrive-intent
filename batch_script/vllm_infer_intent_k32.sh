#!/bin/bash
#SBATCH --job-name=fsdrive_vllm_infer_intent_k32
#SBATCH --partition=a100
#SBATCH --gres=gpu:4
#SBATCH --cpus-per-task=64
#SBATCH --time=1-00:00:00
#SBATCH --output=logs/vllm_infer_intent_k32_%j.out
#SBATCH --error=logs/vllm_infer_intent_k32_%j.err
...
MODEL_PATH=saves/qwen2_vl-2b/sft_intent_k32
SAVE_NAME=eval_results/results_intent_k32.jsonl

python ../tools/patch_vllm_rope_config.py "$MODEL_PATH"

python scripts/vllm_infer.py \
  --model_name_or_path "$MODEL_PATH" \
  --dataset val_cot_motion_intent_k32 \
