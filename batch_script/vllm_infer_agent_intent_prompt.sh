#!/bin/bash
#SBATCH --job-name=fsdrive_vllm_agent_intent_prompt
...
#SBATCH --output=logs/vllm_infer_agent_intent_prompt_%j.out
#SBATCH --error=logs/vllm_infer_agent_intent_prompt_%j.err
...
MODEL_PATH=saves/qwen2_vl-2b/sft_agent_intent_prompt
SAVE_NAME=eval_results/results_agent_intent_prompt.jsonl

python ../tools/patch_vllm_rope_config.py "$MODEL_PATH"

python scripts/vllm_infer.py \
  --model_name_or_path "$MODEL_PATH" \
  --dataset val_cot_motion_agent_intent_prompt \
